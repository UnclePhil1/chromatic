...
export default function GameBoard({ gameState, onGameStateChange, disabled = false }: GameBoardProps) {
  const [draggedRing, setDraggedRing] = useState<{ ring: Ring; fromPole: number } | null>(null)
  const [dragOverPole, setDragOverPole] = useState<number | null>(null)
  const [invalidMove, setInvalidMove] = useState<{ ring: Ring; toPole: number } | null>(null)
  ...
  return (
    <Card className={`p-6 bg-white/95 backdrop-blur-sm h-full ${disabled ? "pointer-events-none opacity-75" : ""}`}>
        ...
        {gameState.poles.map((pole, poleIndex) => (
          ...
          >
            ...
            <div className="relative">
              ...
              <div
                className={`w-3 h-48 bg-amber-600 rounded-t-full ${
                ...
                {pole.map((ring, ringIndex) => {
                  ...
                  return (
                    <div
                      ...
                      className={`
                        w-14 h-5 rounded-full border-2 transition-all duration-200
                        ${RING_COLORS[ring.color]}
                        ${isTopRing && !disabled ? "hover:scale-105 hover:shadow-lg cursor-pointer" : "cursor-not-allowed"}
                        ${isDragging ? "opacity-50 scale-105" : ""}
                        ${isInvalidRing ? "animate-pulse bg-red-500 border-red-600" : ""}
            ...
            <div className="mt-2 text-center">
              <div className="text-sm font-medium text-gray-600">Pole {poleIndex + 1}</div>
              <div className="text-xs text-gray-500">{new Set(pole.map((r) => r.color)).size}/4 colors</div>
              <div className="text-xs text-gray-400">{pole.length}/10 rings</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {gameState.isComplete && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-lg">
          <div className="bg-white p-6 rounded-lg text-center">
            ...
            <p className="text-gray-600">You completed the puzzle in {gameState.moves} moves!</p>
          </div>
        </div>
      )}
    </Card>
